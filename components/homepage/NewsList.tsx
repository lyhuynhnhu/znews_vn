      ...
      <Grid container spacing={4}>
        <Grid size={{ xs: 12, md: 8.5 }} spacing={2}>
          {newsList.map((news: any, idx: number) => (
            <NewsHorizontalCard key={news.id} {...news} isLast={idx === newsList.length - 1} />
          ))}
        </Grid>
        <Grid size={3.5} display={{ xs: 'none', md: 'block' }}>
          <Card variant='outlined' sx={{ py: 4, px: 2 }}>
            ...
            {moreRead.map((news: any) => (
              <Link href={`/news/${news.id}`} style={{ textDecoration: 'none' }} key={news.id}>
                <Box sx={{ display: 'flex', mb: 2 }}>
                  ...
                  <Box sx={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
                    <CustomTypography colorvariant='textSecondary' sx={{ fontSize: '0.7rem' }}>
                      {news.category}
                    </CustomTypography>
                    <CustomTypography
